...
- **Direct imports**: there are no `index.ts` barrels; components under `src/app/components` or `src/app/lib` are imported directly from their files through the `@/` alias.
